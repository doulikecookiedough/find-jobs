...
def _extract_company_after_title(raw_text: str, title: str | None) -> str | None:
    ...
    return candidate if _is_company_line_candidate(candidate) else None


def _extract_salary(lines: list[str]) -> tuple[int | None, int | None, str | None, str | None]:
    """Extract the first salary range found in the job description lines."""
...
def _extract_location(lines: list[str]) -> str | None:
    ...
    for index, line in enumerate(lines[:20]):
        candidate = _extract_location_from_line(index, line, lines)
        if candidate:
            return candidate
    return None
...
def _is_probable_company_name(line: str) -> bool:
    ...
    return not any(invalid_candidates)
...
def _extract_seniority(
    ...
    """Infer seniority from the title first, then fall back to years required."""
    ...
    return _seniority_from_years_required(years_required)


def _extract_salary_from_line(
    line: str,
) -> tuple[int | None, int | None, str | None, str | None]:
    """Extract one salary tuple from a single normalized line."""
...
def _extract_thousands_salary_range(
    ...
    """Extract salary ranges written in thousands with an explicit currency."""

    match = SALARY_THOUSANDS_RANGE_PATTERN.search(line)
    if not match:
        return None, None, None, None
...
def _extract_bare_thousands_salary_range(
    ...
    """Extract salary ranges written in thousands without a currency code."""

    match = SALARY_BARE_THOUSANDS_RANGE_PATTERN.search(line)
    if not match:
        return None, None, None, None
...
def _extract_currency_salary_range(
    ...
    """Extract salary ranges that include a prefixed currency label."""
...
def _extract_plain_salary_range(
    ...
    """Extract plain dollar salary ranges with yearly wording."""
...
def _extract_between_salary_range(
    ...
    """Extract salary ranges written as 'between X and Y'."""

    match = SALARY_BETWEEN_PATTERN.search(line)
    if not match:
        return None, None, None, None
...
def _extract_prefixed_currency_salary_range(
    ...
    """Extract salary ranges that prefix each amount with a currency code."""

    match = SALARY_CURRENCY_RANGE_PATTERN.search(line)
    if not match:
        return None, None, None, None
...
def _extract_location_from_line(index: int, line: str, lines: list[str]) -> str | None:
    """Extract a location candidate from one line in the header block."""
...
def _is_company_line_candidate(candidate: str) -> bool:
    """Return whether a title-adjacent line still looks like a company name."""
...
def _seniority_from_years_required(years_required: float | None) -> str | None:
    """Infer seniority from years of experience when titles are ambiguous."""
